...
class TablePanel(ctk.CTkFrame):
    """Panel que muestra registros en una tabla y permite insertar, editar y eliminar."""
    ...
    def _build(self):
        ...
        # 🔥 BOTONES (solo si NO es bitácora)
        if self.table != "BITACORA_CLIENTES":
            ...
            ctk.CTkButton(top, text="✏ Editar", width=120, height=36,
                          fg_color=COLORS["accent2"],
                          font=ctk.CTkFont("Courier New", 12, "bold"),
                          command=self._edit).pack(side="right", padx=(0, 6), pady=10)

            ctk.CTkButton(top, text="🗑 Eliminar", width=120, height=36,
                          fg_color=COLORS["danger"],
                          hover_color="#c0392b",
                          font=ctk.CTkFont("Courier New", 12, "bold"),
                          command=self._delete).pack(side="right", padx=(0, 6), pady=10)
    ...
    def _edit(self):
        ...
        dlg = FormDialog(self, self.table, self.db, data=data)
        self.wait_window(dlg)
