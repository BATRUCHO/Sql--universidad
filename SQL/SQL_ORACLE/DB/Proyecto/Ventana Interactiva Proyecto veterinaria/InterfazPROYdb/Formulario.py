...
class FormDialog(ctk.CTkToplevel):
    """Ventana modal para insertar o editar un registro."""

    def __init__(self, parent, table, db, data=None):
        super().__init__(parent)
        self.table   = table
        self.db      = db
        self.result  = None
        self.entries = {}
        self.data    = data  # 🔥 None = INSERT | dict = EDIT

        self.title(f"{'Editar' if data else 'Nuevo'} registro — {table}")
    ...
    def _build(self):
        ...
        titulo = "✏ Editar" if self.data else "➕ Insertar"
        ctk.CTkLabel(header, text=f"  {titulo} en {self.table}",
    ...
    def _save(self):
        ...
        try:
            # 🔥 SI ES EDITAR
            if self.data:
                pk_value = self.data[pk]
                self.db.update(self.table, values, pk_value)
